...
def _init_db():
    ...
    for nome,cog,usr,pw,uff,ruolo,email,liv in [
        ...
        db.execute("INSERT OR IGNORE INTO utenti (nome,cognome,username,password,ufficio,ruolo,email,livello) VALUES (?,?,?,?,?,?,?,?)",
                   (nome,cog,usr,pw,uff,ruolo,email,liv))
    for cod,desc,resp in [
        ('GS-2024','Global Service 2024','Geom. Carboni'),
        ('VD-2024','Verde e Diserbo 2024','Geom. Carboni'),
        ('CIM-2024','Servizi Cimiteriali 2024','Resp. Cimiteri'),
        ('PUL-2024','Pulizie Edifici 2024','Resp. Pulizie'),
        ('MAN-2024','Manutenzione Strade 2024','Geom. Carboni'),
    ]:
        db.execute("INSERT OR IGNORE INTO commesse (codice,descrizione,responsabile) VALUES (?,?,?)",(cod,desc,resp))
    for cod,desc,cat,um in [
        ...
        db.execute("INSERT OR IGNORE INTO articoli (codice,descrizione,categoria,unita_misura) VALUES (?,?,?,?)",(cod,desc,cat,um))
    for um in ['cad','m','mq','mc','kg','l','h','gg','conf','box','sacchi','rotoli','pz','ml']:
...
def log(azione, det='', ent='', eid=None):
    try:
        get_db().execute("INSERT INTO log_attivita (utente_id,azione,entita,entita_id,dettaglio,ip) VALUES (?,?,?,?,?,?)",
...
def dashboard():
    ...
    tot = db.execute("SELECT COUNT(*) t, SUM(stato='bozza') bozze, SUM(stato='inviata') inv,"
        " SUM(stato='in_lavorazione') lav, SUM(stato='chiusa') chi, SUM(stato='archiviata') arc"
        " FROM richieste "+flt).fetchone()
    if is_staff:
        rec = db.execute("SELECT r.*,u.nome||' '||u.cognome rn FROM richieste r"
            " LEFT JOIN utenti u ON r.creato_da=u.id ORDER BY r.creato_il DESC LIMIT 10").fetchall()
    else:
        rec = db.execute("SELECT r.*,u.nome||' '||u.cognome rn FROM richieste r"
            " LEFT JOIN utenti u ON r.creato_da=u.id WHERE r.creato_da=? ORDER BY r.creato_il DESC LIMIT 10",(u['id'],)).fetchall()
...
def ana_utenti_salva():
    if session.get('liv') != 'admin': return redirect(url_for('dashboard'))
    db = get_db()
    uid2=request.form.get('id','').strip(); pw_raw=request.form.get('password','').strip()
    nome=request.form.get('nome','').strip(); cognome=request.form.get('cognome','').strip()
    ufficio=request.form.get('ufficio','').strip(); ruolo=request.form.get('ruolo','').strip()
    email=request.form.get('email','').strip(); telefono=request.form.get('telefono','').strip()
    livello=request.form.get('livello','richiedente').strip()
    attivo=1 if request.form.get('attivo','1')=='1' else 0
    if uid2:
        if pw_raw:
            db.execute("UPDATE utenti SET nome=?,cognome=?,ufficio=?,ruolo=?,email=?,telefono=?,livello=?,attivo=?,password=? WHERE id=?",
                (nome,cognome,ufficio,ruolo,email,telefono,livello,attivo,hashlib.sha256(pw_raw.encode()).hexdigest(),uid2))
        else:
            db.execute("UPDATE utenti SET nome=?,cognome=?,ufficio=?,ruolo=?,email=?,telefono=?,livello=?,attivo=? WHERE id=?",
                (nome,cognome,ufficio,ruolo,email,telefono,livello,attivo,uid2))
    else:
        db.execute("INSERT INTO utenti (nome,cognome,username,password,ufficio,ruolo,email,telefono,livello,attivo) VALUES (?,?,?,?,?,?,?,?,?,?)",
            (nome,cognome,request.form.get('username','').strip(),
             hashlib.sha256((pw_raw or 'somica2024').encode()).hexdigest(),
...
def ana_fornitori_salva():
    db=get_db(); fid=request.form.get('id')
...
def api_fornitore(fid):
    ...
    return jsonify(dict(f)) if f else jsonify({})
...
def ana_articoli_salva():
    db=get_db(); aid=request.form.get('id')
    campi=(request.form.get('codice',''),request.form.get('descrizione',''),
           request.form.get('categoria',''),request.form.get('unita_misura',''),request.form.get('note',''))
    if aid: db.execute("UPDATE articoli SET codice=?,descrizione=?,categoria=?,unita_misura=?,note=? WHERE id=?",campi+(aid,))
    else: db.execute("INSERT INTO articoli (codice,descrizione,categoria,unita_misura,note) VALUES (?,?,?,?,?)",campi)
    db.commit()
...
def api_articoli():
    ...
    rows=get_db().execute("SELECT * FROM articoli"+((" WHERE categoria=?" if cat else ""))+" ORDER BY descrizione",([cat] if cat else [])).fetchall()
...
def nuova_richiesta():
    ...
    if request.method=='POST':
        ...
        cur=db.execute("INSERT INTO richieste (numero,data,ufficio_richiedente,referente_id,commessa_id,"
            ...
            (numero,request.form.get('data',date.today().isoformat()),
             ...
             'bozza',u['id']))
        rid=cur.lastrowid
        _salva_articoli_richiesta(db, rid, request.form)
    ...
    return render_template('nuova_richiesta.html', u=u, commesse=commesse,
        ...
        num_prev='ACQ-%d-%04d'%(anno,n+1), aj=aj,
        oggi=date.today().isoformat(), modifica=False, r=None, arti_exist=[],
        categorie=['Settore Edile','Verde e Sfalcio','Segnaletica','Impiantistica','Cimiteriale','Pulizie Edifici','Varie'])

def _salva_articoli_richiesta(db, rid, form):
    descs=form.getlist('desc[]'); ums=form.getlist('um[]')
    qtas=form.getlist('qta[]'); pris=form.getlist('pri[]'); notes=form.getlist('note_riga[]')
...
def modifica_richiesta(rid):
    ...
    if request.method=='POST':
        ...
        _salva_articoli_richiesta(db, rid, request.form)
        db.commit()
        log('modifica','Richiesta '+r['numero']+' modificata','richiesta',rid)
        return redirect(url_for('dettaglio_richiesta',rid=rid))
    anno=datetime.now().year
    n=db.execute("SELECT COUNT(*) FROM richieste WHERE strftime('%Y',data)=?",(str(anno),)).fetchone()[0]
    aj=json.dumps({a['descrizione']:{'um':a['unita_misura'] or '','cat':a['categoria']} for a in articoli},ensure_ascii=False)
...
def dettaglio_richiesta(rid):
    ...
    r=db.execute("SELECT r.*,ut.nome||' '||ut.cognome rn,ut2.nome||' '||ut2.cognome rn2,"
        "c.codice cc,c.descrizione cd FROM richieste r"
        " LEFT JOIN utenti ut ON r.creato_da=ut.id"
...
def lista_rif():
    ...
    rows=db.execute(q,p).fetchall()
    in_attesa=db.execute("SELECT COUNT(*) FROM richieste WHERE stato='inviata'").fetchone()[0]
    return render_template('lista_rif.html', u=u, rows=rows, fc=cerca, fs=stato, in_attesa=in_attesa)
...
def modifica_rif(rfid):
    ...
    if request.method=='POST':
        ...
        _salva_articoli_rif(db, rfid, request.form)
        db.commit()
...
def nuova_rif():
    ...
    r_src=None; arti_src=[]
    if rid_src:
        r_src=db.execute("SELECT rf.*,f.ragione_sociale fn,f.indirizzo fi,f.comune fco,"
            "f.provincia fp,f.partita_iva fpi,f.pec fpec,f.email fe,f.telefono ft,f.referente fr"
            " FROM richieste rf LEFT JOIN fornitori f ON rf.fornitore_id=f.id WHERE rf.id=?",(rid_src,)).fetchone()
    ...
    return render_template('nuova_rif.html', u=u, fornitori=fornitori,
        ...
        um_list=um_list, oggi=date.today().isoformat(), rf=None, modifica=False)

def _salva_articoli_rif(db, rfid, form):
    descs=form.getlist('desc[]'); ums=form.getlist('um[]')
    qtas=form.getlist('qta[]'); notes=form.getlist('note_riga[]')
    for i,desc in enumerate(descs):
        if not desc.strip(): continue
        try: qta=float(qtas[i]) if i<len(qtas) and qtas[i] else None
        except: qta=None
...
def dettaglio_rif(rfid):
    ...
    fornitori_inviati=db.execute("SELECT rf2.*,f2.ragione_sociale fn FROM richieste_fornitori rf2"
        " LEFT JOIN fornitori f2 ON rf2.fornitore_id=f2.id"
        " WHERE rf2.oggetto=? AND rf2.id!=? ORDER BY rf2.id",(rf['oggetto'],rfid)).fetchall()
...
def duplica_rif_multi(rfid):
    ...
    for i in range(1,6):
        ...
        db.commit()
    log('multi_invio','Create '+str(len(creati))+' richieste da '+rf['numero'],'rif',rfid)
    return redirect(url_for('dettaglio_rif',rfid=rfid))
    ...
    if request.method=='POST':
        ...
        cur=db.execute("INSERT INTO ordini (numero,data,rif_id,richiesta_acq_id,fornitore_id,commessa_id,"
            ...
            (numero,request.form.get('data',date.today().isoformat()),
             ...
             'bozza',u['id']))
        oid=cur.lastrowid
        _salva_articoli_ordine(db, oid, request.form)
...
def _salva_articoli_ordine(db, oid, form):
    descs=form.getlist('desc[]'); ums=form.getlist('um[]')
    qtas=form.getlist('qta[]'); prezzi=form.getlist('prezzo[]'); notes=form.getlist('note_riga[]')
    for i,desc in enumerate(descs):
        if not desc.strip(): continue
        try: qta=float(qtas[i]) if i<len(qtas) and qtas[i] else None
        except: qta=None
